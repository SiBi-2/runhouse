...
from runhouse.rns.utils.api import read_resp_data, load_resp_content
...
def load_config(name, endpoint: str = USER_ENDPOINT):
    ...
    if rns_address.startswith("/"):
        ...
        return _load_vault_config(resource_uri, endpoint)

    return _load_local_config(name)


def _load_vault_config(resource_uri, endpoint, ):
    """Load secrets data from Vault for a particular resource URI. By default we allow for reloading shared secrets."""
    resp = requests.get(f"{rns_client.api_server_url}/{endpoint}/{resource_uri}?shared",
